...
HDR_H    = 38
CARD_H   = 74       # un poco más alto para la tira roja + barra top + barra bot
SHIELD_S = 36
PAD_X    = 14
VS_W     = 60       # ancho reservado para la hora central
INFO_H   = 16       # barra inferior (torneo)
BAR_TOP  = 16       # barra superior (grupo)
CORNER   = 8
STRIP_W  = 5
CARD_GAP = 6
DAY_H    = 28
...
_RFFM_LOGO = "https://rffm-cms.s3.eu-west-1.amazonaws.com/large_favicon_87ea61909c.png"
    ...
    for i, campo in enumerate(campos_ordenados):
        ...
        for fecha_iso in sorted(dias_campo.keys()):
            ...
            if y - needed_day < MARGIN:
                y = new_page()
                _draw_field_header(c, MARGIN, y - FIELD_H, campo + " (cont.)", 0)
                y -= FIELD_H + CARD_GAP

            _draw_day_header(c, MARGIN, y - DAY_H, _fecha_label(fecha_iso))
            y -= DAY_H + CARD_GAP
