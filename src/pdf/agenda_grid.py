"""
PDF agenda grid — landscape A4, one page per day, campos as columns.
...
def generar_pdf_agenda_grid(
    ...
    """Returns bytes of a landscape PDF with one page per day, time grid."""
    ...
    # Grid vertical extents (fixed for all pages)
    ...
    def _dia_fin(campos_dia: dict, ini_min: int) -> int:
        ...
        fin = (fin + 29) // 30 * 30
        return min(fin, hora_fin_min)  # hora_fin_min actúa como tope si está fijado
    ...
    for fecha_iso in sorted(por_fecha.keys()):
        ...
        px = grid_h / total_min_dia  # points per minute
        ...
        h = ((hora_ini_dia + 59) // 60) * 60
        while h < hora_fin_dia:
            band_top = grid_top - (h - hora_ini_dia) * px
            band_bot = grid_top - (min(h + 60, hora_fin_dia) - hora_ini_dia) * px
            if (h // 60) % 2 == 0:
                c.setFillColor(HexColor("#F0F4F8"))
                c.rect(grid_x, band_bot, available_w, band_top - band_bot, fill=1, stroke=0)
            h += 60

        # ── Hour and half-hour lines + axis labels ────────────────────────────
        h = ((hora_ini_dia + 59) // 60) * 60
        while h <= hora_fin_dia:
            ly = grid_top - (h - hora_ini_dia) * px
            ...
            c.drawRightString(grid_x - 4, ly - 3, _min_to_hm(h))
            h += 60

        h = ((hora_ini_dia + 59) // 60) * 60 + 30
        while h < hora_fin_dia:
            ly = grid_top - (h - hora_ini_dia) * px
        ...
        for i, campo in enumerate(campo_list):
            ...
            for p in ps:
                ...
                clip_start = max(p_min, hora_ini_dia)
                clip_end   = min(end_min, hora_fin_dia)

                bh = (clip_end - clip_start) * px - 1
                by = grid_top - (clip_end - hora_ini_dia) * px
                bx = cx + 1.5
                bw = campo_w - 3
                ...
                if bh >= SHIELD_MIN_H:
                    # Layout: escudos en los laterales, todo el texto centrado
                    sh_pad  = 4
                    sh_cy   = by + bh / 2
                    ...
                    # Siempre fallback a logo RFFM si no hay escudo
                    ...
                    # Pila de texto centrada verticalmente en el bloque
                    # Líneas de cabecera: torneo (1ª) + grupo (2ª, si existe)
                    meta_sz = 6
                    team_sz = 6
                    vs_sz   = 4.5
                    gap     = 1.5
                    ...
                    # Líneas: torneo / grupo / local – visitante (según el espacio)
                    ty = by + bh - 6 - 2
                    if torneo:
                    ...
                    # Línea única compacta: local – visitante (+ marcador si cabe)
                    sz = 5.5
                    sep = " – "
                    ...
                    vis_t = _trunc(c, vis, "Helvetica-Bold", sz, text_w - sep_w - marc_w - c.stringWidth(loc_t, "Helvetica-Bold", sz))
    ...
    if torneo_legend:
        ...
        # Grid de leyenda: 2 columnas de entradas
        items = list(torneo_legend.items())
        item_h = 24          # altura por entrada
        swatch_w = 32        # bloque de color
        name_w = (PAGE_W - 2 * MARGIN - 30) / 2 - swatch_w - 10  # ancho del texto por columna
        ...
        for idx, (tid, nombre) in enumerate(items):
            ...
            # Left accent strip inside swatch
            c.setFillColor(border_color)
